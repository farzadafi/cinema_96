...
public class Manager {
    ...
    public void confirmCinema() throws SQLException {
        ...
        if(cinemaRepository.confirmCinema("reza") == 0 )
    ...
    public void reserveTicket(String username) throws SQLException {
        ...
        String[] ticketInformation = ticketRepository.getTicketInformation(2);
        if (ticketInformation == null){
            System.out.println("You enter a wrong id Ticket!");
            return;
        }
        filmName = ticketInformation[1];
        System.out.print("How many ticket you want to buy?:");
        numberTickets = input.nextInt();
        if(numberTickets > Integer.parseInt(ticketInformation[3])){
            System.out.println("you enter a number bigger than numberOfTicket we have");
            return;
        }
        if(ticketRepository.updateNumberOfTicket(id,Integer.parseInt(ticketInformation[1]) + numberTickets) == 10 ){
            System.out.println("Something Wrong");
            return;
        }
        priceAll = calcPriceAll(numberTickets,Integer.parseInt(ticketInformation[3]));
    ...
    public int searchOffCode(String off){
        for(int i=0;i<emptyHomeIndexOffCode + 3;i++)
